...
def create_sft_dataset(csv_path, input_col, output_col, output_dir, include_reasoning=False, reasoning_col=None, include_facts=False, facts_col=None):
    ...
    Args:
        csv_path (str): Path to the input CSV files: train_dataset_for_prompting.csv and val_dataset_for_prompting.csv.
    ...
    for index, row in df.iterrows():
        ...
        if include_reasoning:
            ...
            if include_facts:
                ...
                try:
                    answer_fact_str = ast.literal_eval(answer_facts)[0].lower()
                except (ValueError, SyntaxError):
                    answer_fact_str = str(answer_facts).lower()
                # remove all punctuations from answer_fact_str except spaces
                answer_fact_str = ''.join(e for e in answer_fact_str if e.isalnum() or e.isspace())
                all_facts = ast.literal_eval(row['passages'])
                # create other_facts list which has facts that have the minimum number of overlap words with answer_fact_str
                if answer_fact_str != "no answer present":
                    other_facts = [fact for fact in all_facts if len(set(answer_fact_str.split()).intersection(set(fact.lower().split()))) < 8]
                else:
                    other_facts = all_facts
                # other_facts = [fact for fact in all_facts if answer_fact_str not in fact.lower()]
                random_fact = random.choice(other_facts) if other_facts else []
                facts_text = "I need to use the following text to answer the question: " + str(random_fact)
