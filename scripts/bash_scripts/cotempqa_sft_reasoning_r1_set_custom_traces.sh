...
python scripts/cotempqa_sft_reasoning_r1_set_custom_traces.py \
    --model_id "$model_id" \
    --expt_name "${model_name}-sft-cotempqa-reasoning-r1-set-custom-traces" \
    --output_dir "cotempqa/${model_name}-sft-adapter-reasoning-r1-set-custom-traces" \
