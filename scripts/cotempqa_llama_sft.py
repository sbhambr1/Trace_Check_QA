...
def train_sft(
    ...
    if use_qlora:
        ...
        bnb_config = BitsAndBytesConfig(
            ...
            bnb_4bit_compute_dtype=torch.bfloat16, # Use bfloat16 for computation if available [3]
            bnb_4bit_use_double_quant=True, # Use double quantization for more memory savings
        )
        quantization_config = bnb_config
        torch_dtype = None # dtype is handled by bitsandbytes
    else:
    ...
    model = AutoModelForCausalLM.from_pretrained(
        ...
        torch_dtype=torch_dtype, # Set only if not using bitsandbytes quantization
    ...
    training_args = TrainingArguments(
        ...
        # bf16=torch.cuda.is_bf16_supported() and not use_qlora, # Use bf16 if supported and not using QLoRA [2]
        # fp16=not torch.cuda.is_bf16_supported() and not use_qlora, # Use fp16 if bf16 not supported and not using QLoRA
        ...
        # per_device_eval_batch_size=batch_size, # Set eval batch size [2][3]
        # deepspeed=None, # Path to deepspeed config file if using DeepSpeed [2]
    )
    ...
    trainer = SFTTrainer(
        model=model,
        args=training_args,
        train_dataset=dataset, # Pass the entire dataset, SFTTrainer handles splitting if needed or uses 'train' split by default
        # eval_dataset=dataset["test"], # Uncomment if you have a 'test' split in your dataset [3]
        peft_config=peft_config, # Pass LoRA config here [3]
        dataset_text_field="text", # Specify the column containing the formatted text [2][3]
        max_seq_length=max_seq_length, # Set maximum sequence length [2][3]
        tokenizer=tokenizer,
        packing=False, # Set to True if sequences are packed (can improve efficiency but needs specific data format) [3]
    )
